...
def enhancedFeatureExtractorDigit(datum):
    ...
    loop=False
    def dfs(a,b):
        if not 0 <= a < DIGIT_DATUM_WIDTH or not 0 <= b < DIGIT_DATUM_HEIGHT:
            return False
        elif datum.getPixel(a,b)==2 or datum.getPixel(a,b)==1 or (a,b) in matrix.keys():
            return True
        matrix[(a,b)]=True
        return dfs(a-1,b) and dfs(a+1,b) and dfs(a,b-1) and dfs(a,b+1)
    for a in range(DIGIT_DATUM_WIDTH):
        for b in range(DIGIT_DATUM_HEIGHT):
            matrix={}
            if datum.getPixel(a,b)==0 and dfs(a,b)==True:
                loop=True
                break
...
def enhancedPacmanFeatures(state, action):
    ...
    features['score'] = state.getNumFood()
    scaredGhostPos = [ghost.getPosition() for ghost in state.getGhostStates() if ghost.scaredTimer!=0]
    scaredGhost = min(util.manhattanDistance(gPos,pacmanPos) for gPos in scaredGhostPos) if len(scaredGhostPos) else 0
    features["closeToScaredGhosts"] = scaredGhost * factor / 2

    closestFood = min([util.manhattanDistance(pacmanPos, food) for food in foodGrid]) if foodGrid else 0
    closestFood = factor / closestFood if closestFood else closestFood
    features['closeToFood'] = closestFood

    closestGhost = min(util.manhattanDistance(pos, pacmanPos) for pos in ghostPos) if ghostPos else 0
    closestGhost = factor / closestGhost if closestGhost else closestGhost
    features['closeToGhost'] = closestGhost
    return features
