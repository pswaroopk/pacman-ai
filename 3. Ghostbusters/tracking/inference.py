...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        allPossible = util.Counter()
        for oldPos, initProb in self.beliefs.items():
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                allPossible[newPos] += prob*initProb
        allPossible.normalize()
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        """
        particles = []
        particles = [util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, particle))) for particle in self.particles]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = []
        product = list(itertools.permutations(self.legalPositions, self.numGhosts))
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
            weight = 1.0
            for i, val in enumerate(noisyDistances):
                if val is not None:
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            newParticle = list(oldParticle) # A list of ghost positions and loop through and update each entry in newParticle...
            for i, val in enumerate(newParticle):
                newParticle[i] = util.sample(getPositionDistributionForGhost(setGhostPositions(gameState, oldParticle), i, self.ghostAgents[i]))
